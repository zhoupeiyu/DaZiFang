...
@property (nonatomic, assign) NSInteger animateFinish;
@end
...
- (void)setupDefaults
{
    ...
    self.refreshHeader = 70.0;
    self.internalSpacing = 6;
    self.radius = 3;
    self.delay = 0.4;
    self.duration = 0.3;
    self.defaultColor = [UIColor colorFromRGB:0xcdcdcd];
...
//                aCircle.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1.0, 1.0);
...
- (CABasicAnimation *)createAnimationWithDuration:(CGFloat)duration delay:(CGFloat)delay
{
    ...
    anim.toValue = [NSNumber numberWithFloat:1.7f];
...
- (void)doAnimation
{
	self.animateFinish = self.circlesArr.count;
    for (int i = 0; i < self.circlesArr.count; i++) {
...
{
//	[self performSelector:@selector(stopExtraAnimate) withObject:nil afterDelay:5.0];
	[self stopAnimation];
