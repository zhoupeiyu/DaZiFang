...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage imageNamedFromMyBundle:@"navi_back_pressed"] forState:UIControlStateHighlighted];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectZero];
    [_selectButton setImage:[UIImage imageNamedFromMyBundle:@"checkbox_round_white_check"] forState:UIControlStateNormal];
    NSString *imageName = @"checkbox_round_blue_check";
#ifdef Zhongkao
    imageName = @"checkbox_round_Green_check";
#else
    imageName = @"checkbox_round_blue_check";
#endif
    [_selectButton setImage:[UIImage imageNamedFromMyBundle:imageName] forState:UIControlStateSelected];
    ...
    _titleLbl.textColor = [UIColor getFontBlackColor];
    _titleLbl.text = @"预览";
    [_titleLbl sizeToFit];
    
    [_naviBar addSubview:_titleLbl];
...
- (void)configBottomToolBar {
    ...
    [_doneButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorHighlighted forState:UIControlStateHighlighted];
    [_doneButton setTitleColor:_tzImagePickerVc.oKButtonTitleColorDisabled forState:UIControlStateDisabled];
    
    NSString *count = _tzImagePickerVc.selectedModels.count <= 0 ?  @"": [NSString stringWithFormat:@"(%zd)",_tzImagePickerVc.selectedModels.count];
    NSString *title = [NSString stringWithFormat:@"%@%@",_tzImagePickerVc.doneBtnTitleStr,count];
...
- (void)viewDidLayoutSubviews {
    ...
    _selectButton.frame = CGRectMake(self.view.tz_width - 54, 20 + statusBarHeightInterval, 42, 42);
    _titleLbl.centerX = _naviBar.centerX;
    _titleLbl.tz_top = 20 + statusBarHeightInterval;
    _titleLbl.tz_height = 44;
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            [_tzImagePickerVc.view makeToast:title duration:0.25 position:[NSValue valueWithCGPoint:_tzImagePickerVc.view.center]];
            return;
            ...
            if (model.type == TZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                ...
                NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"Select the video when in multi state, we will handle the video as a photo"], _tzImagePickerVc.maxImagesCount];
                [_tzImagePickerVc.view makeToast:title duration:0.25 position:[NSValue valueWithCGPoint:_tzImagePickerVc.view.center]];
            }
        }
    } else {
...
//        photoPreviewCell.previewView.previewVC = self;
//        photoPreviewCell.previewView.imagePicVC = _tzImagePickerVc;
