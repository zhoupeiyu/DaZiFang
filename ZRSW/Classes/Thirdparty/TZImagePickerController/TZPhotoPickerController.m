...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = _model.name;
     UIBarButtonItem *rightButtonItem = [[UIBarButtonItem alloc] initWithTitle:tzImagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
...
- (void)configBottomToolBar {
    ...
    [_previewButton setTitleColor:[UIColor getFontBlackColor] forState:UIControlStateNormal];
    [_previewButton setTitleColor:[UIColor colorFromRGB:0xb5b5b5] forState:UIControlStateDisabled];
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setTitleColor:[UIColor colorFromRGB:0xb5b5b5] forState:UIControlStateNormal];
        [_originalPhotoButton setTitleColor:[UIColor getFontBlackColor] forState:UIControlStateSelected];
        ...
        _originalPhotoLabel.font = [UIFont systemFontOfSize:15];
        _originalPhotoLabel.textColor = [UIColor getFontBlackColor];
    ...
    NSString *count = tzImagePickerVc.selectedModels.count > 0 ? [NSString stringWithFormat:@"(%zd/%zd)",tzImagePickerVc.selectedModels.count,tzImagePickerVc.maxImagesCount] : @"";
    NSString *title = [NSString stringWithFormat:@"%@%@",tzImagePickerVc.doneBtnTitleStr,count];
...
//    _numberImageView.backgroundColor = [UIColor blueColor];
//
...
//    _numberLabel.backgroundColor = [UIColor redColor];
    ...
    _divideLine.backgroundColor = [UIColor colorFromRGB:0xb5b5b5];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr tz_calculateSizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} maxSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)].width + 2;
    if (!tzImagePickerVc.allowPreview) {
        previewWidth = 0.0;
    }
    _previewButton.frame = CGRectMake(15, 0, previewWidth, 44);
    _previewButton.tz_width = !tzImagePickerVc.showSelectBtn ? 0 : previewWidth;
    
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [tzImagePickerVc.fullImageBtnTitleStr tz_calculateSizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} maxSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)].width;
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame) + 9, 0, fullImageWidth + 56, toolBarHeight);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 46, 0, 80, toolBarHeight);
    }
    [_doneButton sizeToFit];
    CGSize downBtnSize = _doneButton.bounds.size;
    _doneButton.frame = CGRectMake(self.view.tz_width - downBtnSize.width - 15, (toolBarHeight - downBtnSize.height) * 0.5, downBtnSize.width, downBtnSize.height);
    
//    _numberImageView.frame = CGRectMake(self.view.tz_width - 56 - 28, 10, 30, 30);
//    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, self.view.tz_width, KSeparatorLineHeight);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.isAllowShowNum = tzImagePickerVc.isAllowShowNum;
    if ([tzImagePickerVc.selectedModels containsObject:model]) {
        NSUInteger imageIndex = [tzImagePickerVc.selectedModels indexOfObject:model];
        model.imageIndex = imageIndex + 1;
    }
    cell.model = model;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            //更新图片顺序
            for (NSUInteger i = 0; i < tzImagePickerVc.selectedModels.count; ++i) {
                TZAssetModel *model_item = tzImagePickerVc.selectedModels[i];
                NSUInteger imageIndex = [tzImagePickerVc.selectedModels indexOfObject:model_item];
                model_item.imageIndex = imageIndex + 1;
            }
            NSMutableArray *refreshIndexPaths = [[NSMutableArray alloc] init];
            for (NSUInteger i = 0; i < selectedModels.count; ++i) {
                NSUInteger seleceedImageIndex = [_models indexOfObject:selectedModels[i]];
                NSIndexPath *indexPath = [NSIndexPath indexPathForItem:seleceedImageIndex inSection:0];
                [refreshIndexPaths addObject:indexPath];
            }
            if (refreshIndexPaths.count > 0) {
                [strongSelf.collectionView reloadItemsAtIndexPaths:refreshIndexPaths];
            }
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSUInteger imageIndex = [tzImagePickerVc.selectedModels indexOfObject:model];
                model.imageIndex = imageIndex + 1;
                //2、刷新总的选中图片（注：刷新的Item应为选中图片在总图的位置）
                NSMutableArray *refreshIndexPaths = [[NSMutableArray alloc] init];
                for (NSUInteger i = 0; i < tzImagePickerVc.selectedModels.count; ++i) {
                    NSUInteger seleceedImageIndex = [_models indexOfObject:tzImagePickerVc.selectedModels[i]];
                    NSIndexPath *indexPath = [NSIndexPath indexPathForItem:seleceedImageIndex inSection:0];
                    [refreshIndexPaths addObject:indexPath];
                }
                if (refreshIndexPaths.count > 0) {
                ...
                [tzImagePickerVc.view makeToast:title duration:0.25 position:[NSValue valueWithCGPoint:tzImagePickerVc.view.center]];
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        [tzImagePickerVc.view makeToast:@"不能选择视频！" duration:0.25 position:[NSValue valueWithCGPoint:tzImagePickerVc.view.center]];
        return;
        if (tzImagePickerVc.selectedModels.count > 0) {
            TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
            NSString *title = [NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"];
            [imagePickerVc.view makeToast:title duration:0.25 position:[NSValue valueWithCGPoint:tzImagePickerVc.view.center]];
        } else {
