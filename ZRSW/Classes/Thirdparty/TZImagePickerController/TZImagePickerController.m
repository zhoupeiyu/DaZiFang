...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        photoPickerVc.isAllowShowNum = self.isAllowShowNum;
...
- (void)viewDidLoad {
   ...
   UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:imagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:imagePickerVc action:@selector(cancelButtonClick)];
    ...
    self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects:space,rightBarButtonItem, nil];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.isFirstAppear && !imagePickerVc.navLeftBarButtonSettingBlock) {
        UIBarButtonItem *backBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle tz_localizedStringForKey:@"Back"] style:UIBarButtonItemStylePlain target:nil action:nil];
        UIBarButtonItem * space = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:nil action:nil];
        space.width = -4;
        self.navigationItem.leftBarButtonItems = [NSArray arrayWithObjects:space,backBarButtonItem, nil];
